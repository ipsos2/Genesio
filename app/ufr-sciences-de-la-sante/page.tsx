...
import { Search, GraduationCap, Users, Target, ShieldCheck, Mail, Phone } from "lucide-react";
...
const teachers: Teacher[] = [
  { name: "Pr. Konan Emmanuel", grade: "Professeur Titulaire", discipline: "Médecine Interne", service: "Doyen de l'UFR", email: "doyen@upsp.ci" },
  { name: "À compléter", grade: "Maître de Conférences", discipline: "Chirurgie Générale", service: "Chirurgie", email: "contact@upsp.ci" },
  { name: "À compléter", grade: "Assistant", discipline: "Pédiatrie", service: "Pédiatrie Médicale", email: "contact@upsp.ci" },
  { name: "À compléter", grade: "Maître-Assistant", discipline: "Gynécologie-Obstétrique", service: "Gynécologie-Obstétrique", email: "contact@upsp.ci" },
];

const grades = ["Tous les grades", "Professeur Titulaire", "Maître de Conférences", "Maître-Assistant", "Assistant"];
const disciplines = ["Toutes les disciplines", "Médecine Interne", "Chirurgie Générale", "Pédiatrie", "Gynécologie-Obstétrique"];
...
const governance = [
  { role: "Doyen de l'UFR", name: "Pr. Konan Emmanuel" },
  { role: "Vice-Doyen chargé des Affaires Académiques", name: "À définir" },
  { role: "Vice-Doyen chargé de la Recherche", name: "À définir" },
  { role: "Secrétariat de l'UFR", name: "À définir" },
  { role: "Conseil de l'UFR", name: "Représentants enseignants et étudiants" },
];
...
export default function UfrPage() {
  ...
  const filtered = useMemo(() => {
    return teachers.filter((t) => {
      ...
      const matchGrade = grade === grades[0] || t.grade === grade;
      const matchDiscipline = discipline === disciplines[0] || t.discipline === discipline;
      return matchQuery && matchGrade && matchDiscipline;
    });
  }, [query, grade, discipline]);
    ...
    <>
      ...
      <section className="container-page py-16 lg:py-20">
        <motion.div
          ...
          className="rounded-2xl border border-medical-100 bg-medical-50/40 p-8 lg:p-12"
        >
          ...
          <p className="mt-4 max-w-3xl text-lg italic leading-relaxed text-navy-900/85">
            ...
            d&rsquo;excellence médicale au service de San Pedro et de la Côte d&rsquo;Ivoire. Chaque étudiant qui franchit
            nos portes porte la responsabilité de cette fondation. »
          </p>
      ...
      <section className="bg-medical-50/40 py-16 lg:py-20">
        <div className="container-page">
          <motion.h2 initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-10 text-3xl font-bold text-navy-900">
            Nos valeurs
          </motion.h2>
      ...
      <section className="container-page py-16 lg:py-20">
        ...
        <div className="overflow-hidden rounded-2xl border border-medical-100">
          {governance.map((g, i) => (
            <div key={g.role} className={`flex items-center justify-between gap-4 px-6 py-4 ${i % 2 === 0 ? "bg-white" : "bg-medical-50/30"}`}>
      ...
      <section className="bg-medical-50/40 py-16 lg:py-20">
        <div className="container-page">
          <motion.h2 initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-8 text-3xl font-bold text-navy-900">
            Annuaire du Corps Enseignant
          </motion.h2>
          ...
          <div className="mb-8 flex flex-col gap-3 sm:flex-row">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full rounded-full border border-medical-100 bg-white py-2.5 pl-11 pr-4 text-sm outline-none focus:border-medical-500"
              />
            </div>
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {filtered.map((t, i) => (
              <motion.div key={t.name + i} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }} className="rounded-2xl border border-medical-100 bg-white p-6">
                ...
                <div className="mt-4 flex items-center gap-2 text-xs text-navy-900/50">
                  <Mail className="h-3.5 w-3.5" /> {t.email}
                </div>
              </motion.div>
            ))}
            {filtered.length === 0 && (
              <p className="col-span-full py-10 text-center text-sm text-navy-900/40">Aucun résultat pour ces critères.</p>
            )}
          </div>
        </div>
      </section>
